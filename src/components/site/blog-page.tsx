...
function buildBlogParagraphs(post: BlogPost) {
  return [
    `${post.intro} Bu konu yalnızca teorik bir bilgi değildir; doğru ürün seçimi, doğru uygulama ve doğru tedarik planı kurulmadığında baskı kalitesi, fire oranı ve teslim süresi doğrudan etkilenir. ${post.focusKeyword} konusunda karar verirken yüzey, baskı adedi, renk beklentisi ve kullanım koşullarını birlikte değerlendirin. Böylece yalnızca bir boya satın almaz, hattınızda daha öngörülebilir sonuç alabileceğiniz bir sistem kurarsınız.`,
    ...
    `${post.applicationContext} Özellikle ticari baskı yapan atölyelerde aynı kaliteyi tekrar edebilmek çok değerlidir. Bir marka için hazırlanan tişört, bir ambalaj üzerindeki logo, bir suni deri yüzeyindeki baskı ya da bir uyarı levhasındaki yazı farklı teknik ihtiyaçlar doğurur. Doğru boya seçildiğinde çizgi netliği artar, renk daha kontrollü görünür, yüzeyden atma riski azalır ve üretim temposu daha rahat yönetilir. Yanlış seçimde ise ürün ucuz görünse bile tekrar baskı, temizlik, bekleme ve müşteri memnuniyetsizliği gibi maliyetler ortaya çıkabilir.`,
    `İyi bir yönlendirme, müşterinin gerçek üretim koşullarını anlamakla başlar. Kullanılan elek numarası, rakle yapısı, baskı adedi, yüzey hazırlığı, kuruma süresi ve ürünün nihai kullanım alanı sorulduğunda amaç süreci zorlaştırmak değildir. Bu bilgiler doğru ürüne hızlı ulaşmak için gereklidir. Serigrafi baskıda küçük bir teknik fark, özellikle yüksek adetli işlerde büyük sonuçlar doğurabilir. Bu nedenle numune, renk beklentisi veya mevcut sorun paylaşıldığında daha isabetli bir ürün grubu seçilebilir.`,
    post.technicalNotes.join(" "),
    `Fiyat tarafında da aynı yaklaşım geçerlidir. ${post.focusKeyword} için tek bir sabit fiyat mantığı çoğu zaman sağlıklı değildir; ürün tipi, ambalaj miktarı, renk, özel formül ihtiyacı ve sevkiyat planı fiyatı etkileyebilir. Rekabetçi teklif alırken ürünün uygulama performansını da dikkate alın. En doğru teklif, ihtiyacınız netleştiğinde hazırlanır. Bu yaklaşım hem gereksiz ürün alımını azaltır hem de uzun vadede daha verimli bir tedarik ilişkisi kurar.`,
    `Düzenli baskı yapan işletmeler için tedarik sürekliliği en az ürün kalitesi kadar önemlidir. Bir atölyenin sık kullandığı boya, yardımcı kimyasal veya kalıp hazırlama ürünü stokta olmadığında üretim planı aksayabilir. Bu nedenle ürün grubu, aylık tüketim, teslimat beklentisi ve sezon yoğunluğu birlikte planlanmalıdır. İstanbul merkezli tedarik yapısıyla hem yerel işletmeler hem de Türkiye genelindeki müşteriler için hızlı iletişim ve planlı sevkiyat seçenekleri değerlendirilebilir.`,
    ...
    `${post.cta} Ürünler sayfasından mevcut grupları inceleyebilir, iletişim sayfasından konum ve telefon bilgilerine ulaşabilir ya da WhatsApp üzerinden doğrudan fiyat ve teknik destek talebi oluşturabilirsiniz. Amaç yalnızca ürün almak değil, serigrafi baskı yapan işletmeler için daha net, daha dayanıklı ve daha sürdürülebilir sonuçlara ulaşmaktır.`,
  ];
}
...
export function buildBlogFaq(post: BlogPost) {
  return [
    ...
    {
      question: "Teknik destek veriliyor mu?",
      answer:
